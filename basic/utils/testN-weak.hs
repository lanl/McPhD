...
{-| test the performance of black-hole as we vary the number of threads and 
 - the workload to maintain constant work/thread. -}
...
runARun rundir commStr n nCores = do
  let nPtcls = 1000000 * nCores
      comm = printf commStr nPtcls rundir nCores n nCores rundir nCores n
...
runOneVal :: FilePath -> -- directory for output files
          ...
          Int ->  -- number of cores
          IO ()
runOneVal dir -- fPrefix
       summOnly nRuns nCores = do
  -- cl to run
  let commStr = "time ./black-hole -i ../../data/p.7 -n %i -c 500 -s 4523645 +RTS -s%s/N_%i_gc_%i -N%i -RTS 2>&1 | tee %s/N_%i__run_%i"
